...
bool AnagramFinder<Dict>::checkWord(const string &word, const string &test) {
    ...
    // Create two arrays to store the frequency of each character in both words
    int freq1[256] = {0}; // Frequency array for the word
    int freq2[256] = {0}; // Frequency array for the test string

    // Increment the frequency count for each character in the word
    for (char c : word) {
        freq1[static_cast<unsigned char>(c)]++;
    }

    // Increment the frequency count for each character in the test string
    for (char c : test) {
        freq2[static_cast<unsigned char>(c)]++;
    }

    // Compare the frequency arrays
    for (int i = 0; i < 256; ++i) {
        if (freq1[i] != freq2[i]) {
            return false; // If any frequency counts are different, the words are not anagrams
        }
    }

    // If all frequency counts match, the words are anagrams
    return true;
}
