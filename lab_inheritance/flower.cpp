...
// Update Flower constructor to allocate memory dynamically for Drawable objects
...
// Update Flower destructor to delete dynamically allocated memory
Flower::~Flower() {
    delete stem;
    delete pistil;
    delete leaf;
...
void Flower::draw(PNG* canvas) const
{
    ...
    for (int x = 0; x < PISTIL_RADIUS; x++)
    {
        ...
        drawPedals(canvas, dynamic_cast<Circle*>(pistil)->center(), x, y);
    }
}
